...
echo BFVR-VR-debug started %DATE% %TIME% > D:\BF1942\vr-logs\result.txt
rem No debugger here: BFVR skips its CreateDevice hook when one is attached.
D:\BF1942\BFVR\BFVR.exe --d3d8to9-observer-probe --d3d8-openxr-presentation-probe --weapon-motion-probe --run-until-stopped > D:\BF1942\vr-logs\bfvr-output.txt 2>&1
