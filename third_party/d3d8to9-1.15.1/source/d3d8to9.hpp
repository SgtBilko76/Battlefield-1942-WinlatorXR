...
public:
	Direct3DDevice8(Direct3D8 *d3d, IDirect3DDevice9 *ProxyInterface, DWORD BehaviorFlags, D3DFORMAT ZBufferFormat, BOOL EnableZBufferDiscarding, bool IsExtendedDevice);
	~Direct3DDevice8();

	IDirect3DDevice9 *GetProxyInterface() const { return ProxyInterface; }
	bool UsesExtendedDevice() const { return IsExtendedDevice; }
	HRESULT GetVertexShaderIdentity(
		DWORD Handle,
...
private:
	...
	const bool IsExtendedDevice = false;
...
public:
	...
	void SetEmulatedManagedPool(DWORD OriginalUsage) { EmulatesManagedPool = true; EmulatedManagedUsage = OriginalUsage; }
...
private:
	...
	bool EmulatesManagedPool = false;
	DWORD EmulatedManagedUsage = 0;
};
