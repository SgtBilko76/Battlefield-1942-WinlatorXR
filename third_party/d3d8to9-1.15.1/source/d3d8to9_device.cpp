...
struct VertexShaderInfo
{
	...
	ULONGLONG OriginalFunctionHash = 0;
	DWORD OriginalFunctionByteCount = 0;
	DWORD CreationOrdinal = 0;
	BOOL Programmable = FALSE;
};

namespace
{
ULONGLONG HashShaderBytecode(
	const DWORD *Function,
	DWORD ByteCount)
{
	constexpr ULONGLONG OffsetBasis = 14695981039346656037ULL;
	constexpr ULONGLONG Prime = 1099511628211ULL;
	ULONGLONG Hash = OffsetBasis;
	const BYTE *const Bytes =
		reinterpret_cast<const BYTE *>(Function);
	for (DWORD Index = 0; Index < ByteCount; ++Index)
	{
		Hash ^= Bytes[Index];
		Hash *= Prime;
	}
...
Direct3DDevice8::Direct3DDevice8(Direct3D8 *d3d, IDirect3DDevice9 *ProxyInterface, DWORD BehaviorFlags, D3DFORMAT ZBufferFormat, BOOL EnableZBufferDiscarding, bool IsExtendedDevice) :
	D3D(d3d), ProxyInterface(ProxyInterface), IsExtendedDevice(IsExtendedDevice), ZBufferDiscarding(EnableZBufferDiscarding)
{
...
HRESULT STDMETHODCALLTYPE Direct3DDevice8::CreateAdditionalSwapChain(D3DPRESENT_PARAMETERS8 *pPresentationParameters, IDirect3DSwapChain8 **ppSwapChain)
{
	...
	*ppSwapChain = nullptr;
	...
	BFVRD3D8To9AdjustPresentParameters(&PresentParams);
...
HRESULT STDMETHODCALLTYPE Direct3DDevice8::CreateTexture(UINT Width, UINT Height, UINT Levels, DWORD Usage, D3DFORMAT Format, D3DPOOL Pool, IDirect3DTexture8 **ppTexture)
{
	...
	*ppTexture = nullptr;
	...
	const bool EmulateManagedPool =
		IsExtendedDevice && Pool == D3DPOOL_MANAGED;
	const DWORD OriginalUsage = Usage;
	if (EmulateManagedPool)
	{
	...
	if (EmulateManagedPool)
		BFVRD3D8To9RecordManagedTranslation(
			BFVRD3D8To9ManagedResourceKind::Texture,
			hr);
	if (FAILED(hr))
		return hr;

	auto Texture = ProxyAddressLookupTable->FindAddress<Direct3DTexture8>(TextureInterface);
	if (EmulateManagedPool)
		Texture->SetEmulatedManagedPool(OriginalUsage);
	*ppTexture = Texture;
...
HRESULT STDMETHODCALLTYPE Direct3DDevice8::CreateVolumeTexture(UINT Width, UINT Height, UINT Depth, UINT Levels, DWORD Usage, D3DFORMAT Format, D3DPOOL Pool, IDirect3DVolumeTexture8 **ppVolumeTexture)
{
	...
	*ppVolumeTexture = nullptr;
	...
	if (EmulateManagedPool)
		BFVRD3D8To9RecordManagedTranslation(
			BFVRD3D8To9ManagedResourceKind::VolumeTexture,
			hr);
	if (FAILED(hr))
		return hr;

	auto Texture = ProxyAddressLookupTable->FindAddress<Direct3DVolumeTexture8>(TextureInterface);
	if (EmulateManagedPool)
		Texture->SetEmulatedManagedPool(OriginalUsage);
...
HRESULT STDMETHODCALLTYPE Direct3DDevice8::CreateCubeTexture(UINT EdgeLength, UINT Levels, DWORD Usage, D3DFORMAT Format, D3DPOOL Pool, IDirect3DCubeTexture8 **ppCubeTexture)
{
	...
	*ppCubeTexture = nullptr;
	...
	if (EmulateManagedPool)
		BFVRD3D8To9RecordManagedTranslation(
			BFVRD3D8To9ManagedResourceKind::CubeTexture,
			hr);
	if (FAILED(hr))
		return hr;

	auto Texture = ProxyAddressLookupTable->FindAddress<Direct3DCubeTexture8>(TextureInterface);
	if (EmulateManagedPool)
		Texture->SetEmulatedManagedPool(OriginalUsage);
...
HRESULT STDMETHODCALLTYPE Direct3DDevice8::CreateVertexBuffer(UINT Length, DWORD Usage, DWORD FVF, D3DPOOL Pool, IDirect3DVertexBuffer8 **ppVertexBuffer)
{
	...
	*ppVertexBuffer = nullptr;
	...
	if (EmulateManagedPool)
		BFVRD3D8To9RecordManagedTranslation(
			BFVRD3D8To9ManagedResourceKind::VertexBuffer,
			hr);
	if (FAILED(hr))
		return hr;

	auto Buffer = ProxyAddressLookupTable->FindAddress<Direct3DVertexBuffer8>(BufferInterface);
	if (EmulateManagedPool)
		Buffer->SetEmulatedManagedPool(OriginalUsage);
...
HRESULT STDMETHODCALLTYPE Direct3DDevice8::CreateIndexBuffer(UINT Length, DWORD Usage, D3DFORMAT Format, D3DPOOL Pool, IDirect3DIndexBuffer8 **ppIndexBuffer)
{
	...
	*ppIndexBuffer = nullptr;
	...
	if (EmulateManagedPool)
		BFVRD3D8To9RecordManagedTranslation(
			BFVRD3D8To9ManagedResourceKind::IndexBuffer,
			hr);
	if (FAILED(hr))
		return hr;

	auto Buffer = ProxyAddressLookupTable->FindAddress<Direct3DIndexBuffer8>(BufferInterface);
	if (EmulateManagedPool)
		Buffer->SetEmulatedManagedPool(OriginalUsage);
...
HRESULT STDMETHODCALLTYPE Direct3DDevice8::CreateVertexShader(const DWORD *pDeclaration, const DWORD *pFunction, DWORD *pHandle, DWORD Usage)
{
	...
	*pHandle = 0;
	...
	if (pFunction != nullptr)
	{
		...
		if (D3DXGetShaderSize != nullptr)
		{
			const UINT FunctionByteCount =
				D3DXGetShaderSize(pFunction);
			constexpr UINT MaximumFunctionByteCount = 64 * 1024;
			if (FunctionByteCount >= sizeof(DWORD) * 2 &&
				FunctionByteCount <= MaximumFunctionByteCount &&
				(FunctionByteCount % sizeof(DWORD)) == 0)
			{
				OriginalFunctionByteCount = FunctionByteCount;
				OriginalFunctionHash = HashShaderBytecode(
					pFunction,
					OriginalFunctionByteCount);
			}
		}
		...
		{
#ifndef D3D8TO9NOLOG
			LOG << "> Calling D3DXAssembleShader ..." << std::endl;
#endif
...
#ifndef D3D8TO9NOLOG
			LOG << "> D3DXAssembleShader returned " << std::hex << hr << std::dec << "." << std::endl;
#endif
		}
		else
		{
...
#ifndef D3D8TO9NOLOG
		LOG << "> Calling IDirect3DDevice9::CreateVertexShader ..." << std::endl;
#endif
		hr = ProxyInterface->CreateVertexShader(static_cast<const DWORD *>(Assembly->GetBufferPointer()), &ShaderInfo->Shader);
#ifndef D3D8TO9NOLOG
		LOG << "> IDirect3DDevice9::CreateVertexShader returned " << std::hex << hr << std::dec << "." << std::endl;
#endif
	...
	ShaderInfo->OriginalFunctionHash = OriginalFunctionHash;
	ShaderInfo->OriginalFunctionByteCount =
		OriginalFunctionByteCount;
	ShaderInfo->Programmable =
		pFunction != nullptr ? TRUE : FALSE;

	if (SUCCEEDED(hr))
	{
#ifndef D3D8TO9NOLOG
		LOG << "> Calling IDirect3DDevice9::CreateVertexDeclaration ..." << std::endl;
#endif
		hr = ProxyInterface->CreateVertexDeclaration(VertexElements, &ShaderInfo->Declaration);
#ifndef D3D8TO9NOLOG
		LOG << "> IDirect3DDevice9::CreateVertexDeclaration returned " << std::hex << hr << std::dec << "." << std::endl;
#endif

		if (SUCCEEDED(hr))
		{
			ShaderInfo->CreationOrdinal =
				++VertexShaderCreationOrdinal;
...
#ifndef D3D8TO9NOLOG
			LOG << "> Returning D3D8 vertex-shader handle " << std::hex << *pHandle << std::dec << "." << std::endl;
#endif
		}
		else
		{
...
HRESULT Direct3DDevice8::GetVertexShaderIdentity(
	DWORD Handle,
	BFVRD3D8To9VertexShaderIdentity& Identity) const
{
	if ((Handle & 0x80000000) == 0 ||
		VertexShaderHandles.find(Handle) ==
			VertexShaderHandles.end())
	{
		return D3DERR_INVALIDCALL;
	}

	const DWORD HandleMagic = Handle << 1;
	const VertexShaderInfo *const ShaderInfo =
		reinterpret_cast<const VertexShaderInfo *>(HandleMagic);
	Identity = {};
	Identity.size = sizeof(Identity);
	Identity.version = BFVR_D3D8TO9_SHARED_BRIDGE_VERSION;
	Identity.d3d8Handle = Handle;
	Identity.programmable = ShaderInfo->Programmable;
	Identity.originalFunctionByteCount =
		ShaderInfo->OriginalFunctionByteCount;
	Identity.creationOrdinal = ShaderInfo->CreationOrdinal;
	Identity.originalFunctionHash =
		ShaderInfo->OriginalFunctionHash;
	return D3D_OK;
}
